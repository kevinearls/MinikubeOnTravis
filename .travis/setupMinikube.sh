...
until kubectl --namespace storage get statefulset elasticsearch --output=jsonpath='{.status.readyReplicas}' | grep --quiet 1; do sleep 1;echo "waiting for elasticsearch to be available"; kubectl get statefulsets --namespace storage; done
...
until kubectl --namespace storage get statefulset cassandra --output=jsonpath='{.status.readyReplicas}' | grep --quiet 3; do sleep 1;echo "waiting for cassandra to be available"; kubectl get statefulsets --namespace storage; done
...
until kubectl --namespace storage get deployment simple-prod-query --output=jsonpath='{.status.readyReplicas}' | grep --quiet 1; do sleep 1;echo "waiting for jaeger simple-prod deployment to be available"; kubectl get deployments --namespace storage; done
